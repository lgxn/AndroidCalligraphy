package com.applee.MyColorDialog;
...
public class ColorPickerDialog extends Dialog
{
		...
		protected void onCreate(Bundle savedInstanceState)
		{
				...
				ColorPickerView myView = new ColorPickerView(context, height,
								width);
		...
		private class ColorPickerView extends View
		{
				...
				public ColorPickerView(Context context, int height, int width)
				{
						...
						// 渐变色环参数
						mCircleColors = new int[]{0xFFFF0000, 0xFFFF00FF,
						...
						// 黑白渐变参数
						mRectColors = new int[]{0xFF000000,
										mCenterPaint.getColor(), 0xFFFFFFFF};
						mRectPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
						mRectPaint.setStrokeWidth(5);
						rectLeft = -r - mPaint.getStrokeWidth() * 0.5f;
						rectTop = r + mPaint.getStrokeWidth() * 0.5f
										+ mLinePaint.getStrokeMiter() * 0.5f
										+ 15;
				...
				protected void onDraw(Canvas canvas)
				{
						...
						canvas.save();
						// 移动中心
						canvas.translate(mWidth / 2, mHeight / 2 - 80);
						...
						rectShader = new LinearGradient(rectLeft, 0, rectRight,
										0, mRectColors, null,
										Shader.TileMode.MIRROR);
						mRectPaint.setShader(rectShader);
						canvas.drawRect(rectLeft, rectTop, rectRight,
										rectBottom, mRectPaint);

						float offset = mLinePaint.getStrokeWidth() / 2;

						canvas.drawLine(rectLeft - offset,
										rectTop - offset * 2,
										rectLeft - offset, rectBottom + offset
														* 2, mLinePaint);// 左

						canvas.drawLine(rectLeft - offset * 2,
										rectTop - offset, rectRight + offset
														* 2, rectTop - offset,
										mLinePaint);// 上

						canvas.drawLine(rectRight + offset, rectTop - offset
										* 2, rectRight + offset, rectBottom
										+ offset * 2, mLinePaint);// 右

						canvas.drawLine(rectLeft - offset * 2, rectBottom
										+ offset, rectRight + offset * 2,
										rectBottom + offset, mLinePaint);// 下
						...
						float height = mTextPaint.getTextSize();
						canvas.drawText(String.format("COLOR : 0x%H",
										mCenterPaint.getColor()), rectLeft
										- offset * 2, rectBottom + offset * 2
										+ height, mTextPaint);
						canvas.drawText(String.format(
										"ARGB : (%d, %d, %d, %d)", a, r, g, b),
										rectLeft - offset * 2, rectBottom
														+ offset * 2 + height
														* 2, mTextPaint);
						canvas.restore();
				}
				...
				public boolean onTouchEvent(MotionEvent event)
				{
						...
						float y = event.getY() - mHeight / 2 + 80;
						...
						switch (event.getAction())
						{
								...
								case MotionEvent.ACTION_MOVE :
										...
										{// down按在渐变色环内, 且move也在渐变色环内
												...
												mCenterPaint.setColor(interpCircleColor(
																mCircleColors,
																unit));
												if (debug)
														Log.v(TAG, "色环内, 坐标: "
										...
										{// down在渐变方块内, 且move也在渐变方块内
												mCenterPaint.setColor(interpRectColor(
																mRectColors, x));
										}
										if (debug)
												Log.v(TAG,
								...
								case MotionEvent.ACTION_UP :
										...
										{// 点击在中心圆, 且当前启动在中心圆
												if (mListener != null)
												{
														mListener.colorChanged(mCenterPaint
																		.getColor());
				 ...
				 * @return
				 */
				private boolean inColorCircle(float x, float y,
				...
				{
						double centerCircle = Math.PI * centerRadius
										* centerRadius;
						double fingerCircle = Math.PI * (x * x + y * y);
				...
				private int interpCircleColor(int colors[], float unit)
				{
						...
						// now p is just the fractional part [0...1) and i is
						// the index
						int c0 = colors[i];
						int c1 = colors[i + 1];
